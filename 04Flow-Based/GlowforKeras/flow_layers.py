...
class Squeeze(Layer):
    ...
    def __init__(self, factor=2, **kwargs):
        ...
        self.factor = factor
    def call(self, inputs):
        # K.int_shape(inputs):查看张量inputs各个维度的大小
        height, width, channel = K.int_shape(inputs)[1:]
        assert height % self.factor == 0 and width % self.factor == 0
        inputs = K.reshape(inputs, (-1, height//self.factor, self.factor, width//self.factor, self.factor, channel))
        # 在张量中置换轴
        inputs = K.permute_dimensions(inputs, (0, 1, 3, 2, 4, 5))
        x_outs = K.reshape(inputs, (-1, height//self.factor, width//self.factor, channel*self.factor**2))
        return x_outs
    def compute_output_shape(self, input_shape):
        height, width, channel = input_shape[1:]
        return  (None, height//self.factor, width//self.factor, channel*self.factor**2)
    def inverse(self):
