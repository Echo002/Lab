...
def squeeze2d(inputs, factor=2):
    # x各个空间维度切割成两分，转置到通道维度上去
    assert factor >= 1
    # assert 条件为true正常执行，条件为false触发异常
    if factor == 1:
        return inputs
    ...
    # [-1, height//factor, width//factor, channels, factor, factor]
...
def logistic_logcdf(inputs, mean, logs, name="logistic_logcdf"):
    with tf.variable_scope(name):
        z = (inputs - mean) * tf.exp(-logs)
